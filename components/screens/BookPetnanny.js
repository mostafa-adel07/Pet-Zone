    ...
    <SafeAreaView style={styles.container}>
      ...
      <View style={styles.container}>
        <FlatList
          data={petnannys1}
          renderItem={({ item }) => {
            return  <Card style={styles.card} onPress={()=>navigation.navigate('petnannydetails',{petnannyid:item._id})}>
            <Card.Cover
            key={item._id}
            style={styles.cover}
            source={{ uri: item.profilePicture}}
          />
            <Text style={styles.title1}> petnannyName: {item.serviceProvider.userName}</Text>
